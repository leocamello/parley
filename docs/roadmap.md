...
- **Sprint 13 — `parley add <pkg> <constraint>`.** Issue #15. This list's top item, staged undisplaced — the third consecutive sprint the ranking won on its own merits. Its stated trigger was **the ruling, not the calendar**, and the ruling has fired: [§8 decision 49](design/architecture.md#8-decision-log) — Parley never re-renders a file it did not write, so `add` inserts one clause into a recognized cascade, preserves every other byte, verifies the edit through the settled reader, and refuses without touching the file when it does not recognize the shape. Adding holds the existing pins (`holding:` in its third application), and the sprint is atomic: a failed `add` leaves the project byte-identical. Operator direction: S3 (comments and formatting survive) is the scenario decision 49 exists for — a round-trip implementation fails it, and that is the point. Sprint 12's deleted-held-pin gap rides along as S13.
...
| 13 | `parley add` | The ruling that gated it is made — [§8 decision 49](design/architecture.md#8-decision-log): a `ManifestBuilder` round-trip is disqualified, because `Package.st` is executable Smalltalk the author owns and the manifest is a lossy projection of it. Recognize or refuse, verify by re-reading, roll back on failure. Staged, issue #15. |
