...
- *Nothing in flight.* Sprint 15 closed, and **the v1.0 line is complete** (§5). Every remaining item below is either trigger-gated or on the 2.0 axis, so for the first time the list has no item that simply wins by being next — Gate C's first question is now whether a trigger has fired, not what is at the top.
...
- **Exit `70` is honest about failure and imprecise about blame** ([§8 decision 55](design/architecture.md#8-decision-log), Sprint 15 close-out). The decision-52 narrowing sends a seed-gathering error outside the two declared boundaries to `CommandLine`'s exit `70`, which reads *this is a defect in Parley, not in your project*. Against the alternative it replaced — a `#noVersions` conflict blaming the package — that is the right trade and it is what shipped. But the state that now reaches it is often the operator's own: a `parley.lock` that is a **directory** is neither a well-formed lock nor a Parley bug, and the tool answers it by claiming a defect in itself. *Severity: low — an honest fail-stop with the wrong return address, no bad artifact written, and the verb refuses rather than proceeding.* **Consumer constraint:** an exit `70` naming the lock read may mean a malformed *lock file* rather than a Parley defect; the first check is `ls -l parley.lock`. The fix is `pinnedResolution` validating **readability** alongside shape — that the path is a regular readable file before its bytes are parsed — which grows the `LockError` wording set and is a contract change on a settled boundary (Doc E §4.1), not a two-identifier repair. The trigger is the next sprint that touches the §4.1 lock boundary, the same trigger shape decision 52 rode in on.
