...
- **Sprint 20 — the verbs that close the grammar.** ⏳ **STAGED 2026-08-12**, issue [#22](https://github.com/leocamello/parley/issues/22), against **910 laws**. **§2 item 5, undisplaced — the tenth consecutive sprint the ranking won on its own merits**, `deferred 0×`. `remove`, `search`, `info`, `outdated`. §8 decisions **67**, **79**, **80**, landed before RED at `9b5d443` with the docs citing them. Two riders, neither leading: the usage-error first lines (decision 79, which also decouples `--help` from the usage-error answer) and the **F19 carried gap** below.
