...
- **Sprint 20 — the verbs that close the grammar.** ✅ **DELIVERED 2026-08-12**, issue [#22](https://github.com/leocamello/parley/issues/22) closed, commit `91586dc`, **910 → 983 laws**. Staged at Gate C on 2026-08-12 against **910 laws**. **§2 item 5, undisplaced — the tenth consecutive sprint the ranking won on its own merits**, `deferred 0×`. `remove`, `search`, `info` and `outdated` ship, and **no new class and no new `src/` directory** were added to do it. §8 decisions **67**, **79**, **80** landed before RED at `9b5d443` with the docs citing them; **81** and **82** were ruled mid-sprint, and **67 and 79 both carry marked in-place amendment clauses** because their first drafts were wrong in the repository (see below). Two riders, neither leading: the usage-error first lines (decision 79, which also decouples `--help` from the usage-error answer) and the **F19 carried gap**, now **cleared**. **Ruled pre-RED, 2026-08-12** ([decision 81](design/architecture.md#8-decision-log)): the tool reaches usage by **six** routes, not three — `verbRowFor:withSource:` had enumerated **four** verb-grammar grounds in its own comment since Sprint 16, and *a known verb given arguments it does not take* is independent (probed: `resolve --help` answers usage **even with a source wired**). The two remaining routes are decided outside the verb grammar — both source flags, and an unrecognized `--layout` — and **none of the six composes its reason at the site that refuses**: all six are one declared table read twice, the `sourceFlags`/`verbRows` idiom. Decision 79's *three grounds*, *seven-line block* and *three positional laws* were **all three wrong** and are corrected in place. **Ruled at Gate A** ([decision 82](design/architecture.md#8-decision-log)): a **transitive** pin has no constraint of the operator's, so `outdated` renders a **declared marker** in its allowed column rather than letting *unconstrained* collapse into *the newest is allowed* — and [decision 67](design/architecture.md#8-decision-log) is amended to say **which** join a deletion owns (a clause owns the join before it, except the first, which owns the join after it — probed: the alternative leaves `pkg;`, which does not parse). **Delivered, walked through the shipped binary at Gate B:** four grounds that answered a byte-identical 476 B block now answer **six distinct first lines**, every one of them *reason + the six-line block* byte-for-byte, while `--help` stays six lines at exit `0` with no reason line; `search` answers over `--source` and `--git` and **refuses over `--index` at exit 1 spawning nothing**; `info` reaches a package **outside the seeded closure** and marks retired releases with the owner's reason; `outdated` reports both columns without conflating them, the marker for a transitive pin, and a retired newer release in **neither** column; `remove` takes a cascade's last clause **with its separator** and leaves a refused file byte-identical. **F19 cleared on all three branches** — cold, warm, and warm-over-a-moved-index each now put **0 bytes of git's own text on both streams**, and a missing repository still answers exit `1` with the `SourceError` naming `git clone --quiet …` and code `128`. Close-out findings: **F22** (a cross-artifact contradiction is caught only by a second party re-deriving the number — including inside the artifact written to correct one) and **F23** (a toolchain constraint recorded five times still cost a run, and the remedy applied was a sixth copy).

### Recently delivered

- **Sprint 19 — the commands stop shouting, and the project remembers where its index is.** ✅ **DELIVERED 2026-08-11**, issue #21 closed, commit `89ab038`, **876 → 910 laws**. Staged at Gate C on 2026-08-10 against **876 laws**. **§2 items 3 AND 4, staged together and undisplaced — the ninth consecutive sprint the ranking won on its own merits.** Not a displacement: the two are adjacent, both at deferred 0×, and nothing below them is jumped. They are paired because **Gate C's probes shrank both** — `exec` was carved out of item 3 (its GC message is on **stderr**, from the image load Parley composes, and its plumbing shares one stream with the author's program, so separating them is the deferred buffering mechanism by name), and item 4's installation half was found **already delivered** in Sprint 17, leaving only the configuration half. `Publisher` and `Installer` gain a stdout redirect on their composed plumbing lines ([decision 63](design/architecture.md#8-decision-log)); `Parley.Configuration` (`src/exec/`) reads a project-root `parley.config.st` in the settled micro-format under `#'parley-config' 1`, **not** under `.parley/` because that directory is regenerable state, and **an explicit flag always wins** ([decisions 64](design/architecture.md#8-decision-log) and [65](design/architecture.md#8-decision-log)). The **Sprint 18 carried gap rides along**: a recovered anchor means the load failed whatever the file recorded, closing a fail-wrong where a `Package.st` with a raising `define:` exited `0` ([decision 76](design/architecture.md#8-decision-log)). **The `exec` carve-out was re-ruled pre-RED, 2026-08-11** ([decision 77](design/architecture.md#8-decision-log)): the toolchain's documented `-g` (`--no-gc-message`) suppresses the GC chatter at the source — probed: stderr drops to zero bytes, both streams and the exit code untouched — so `exec` goes quiet by flag, the fragment is no longer kept, and the carve-out's two-route ground is recorded as the enumeration it was. **Delivered:** `publish` 592 → **21** stdout bytes, `install` 324 → **11**, `exec` stderr 67 → **31** with stdout byte-identical and the exit code passed through, the double-`define:` fail-wrong closed at exit `1`, and `rm -rf .parley` followed by a flagless `resolve` now works. Ruled at the close-out: [decision 78](design/architecture.md#8-decision-log) — the redirect belongs to each composer, two may own it, a third means extract — and the config is consulted for **every verb that runs**, so `init`, `check` and `tree` answer the config diagnosis beside a malformed file while `--version`/`--help` stay at exit `0` (Doc E §4.2). **Close-out gap:** `GitIndexSource` was never in the domain the sprint enumerated and still prints `Cloning into '<cache path>'` to stderr — **carried gap, ranked, unscheduled** (method finding **F19**).
...
2. ~~**Writing your first `Package.st`**~~ — ✅ **DELIVERED in Sprint 18** (issue #20, commit
   ...
   the note above.
3. ~~**Running your first command**~~ — ✅ **DELIVERED in Sprint 19** (issue #21, commit `89ab038`), paired with item 4, *deferred 0×, undisplaced.* **Not fully closed: `GitIndexSource` was outside the domain the sprint enumerated and still prints git's own two lines to stderr — see Carried gaps, method finding F19.** Every successful verb prints more foreign
...
4. ~~**Getting Parley, and telling it where your index is**~~ — ✅ **DELIVERED in Sprint 19**
   (issue #21, commit `89ab038`), paired with item 3, *deferred 0×, undisplaced.* **Re-probed at Sprint 19's Gate C, 2026-08-10, and the earlier
...
5. ~~**The verbs that close the grammar**~~ — ✅ **DELIVERED in Sprint 20** (issue #22, commit
   `91586dc`), *deferred 0×, staged undisplaced — the tenth consecutive sprint the ranking won
   on its own merits.* `remove`, `search`, `info` and `outdated`, all four present in all seven
   of the package managers this project benchmarks against, shipped with **no new class and no
   new `src/` directory**. `remove` was the declared other half of `add` and its deferral
   trigger had fired (§3). §8 decisions **67** (as amended at Gate A — a clause owns the join
   *before* it, except the first, which owns the join *after* it), **80** (`search` refuses over
   `--index` rather than answering over the closure it happens to reach), **81** (six usage
...
- ~~**`--git` still prints text Parley did not write, and it is the sprint's own defect at the composer nobody listed.**~~ — ✅ **CLEARED in Sprint 20** (issue #22, commit `91586dc`, S20–S22), as the rider it was scheduled as. `GitIndexSource>>refreshCommand`'s two composed lines gained `--quiet` and nothing else. **Walked through the shipped binary at Sprint 20's Gate B, all three branches, streams captured separately:** a **cold** cache, a **warm** cache with the index unchanged, and a **warm cache whose index has moved** — the case the original record missed — each now put **0 bytes of git's own text on stdout and 0 on stderr**, leaving only Parley's own answer. **Silencing the chatter silenced neither failure:** a missing repository still answers exit `1`, the settled `SourceError` naming `git clone --quiet …` and code `128`, with git's own `fatal:` still reaching the operator on stderr. The three F19 stream-capture laws are **new coverage** — no law captured streams on a `--git` verb before this sprint, which is exactly why the gap shipped. *Original record retained below, because the understatement is the more useful half of it.* *Recorded at the Sprint 19 close-out (method finding **F19**); **description corrected at Sprint 20's Gate C, 2026-08-12, and scheduled there as a rider**.* **The original entry understated it, and the understatement is the more useful record:** it read *"two lines to stderr"* and gave the consumer constraint as *"a script capturing stderr will collect them"*, which describes the **cold-cache clone only**. `refreshCommand` composes **two** lines, and the second is the common one — every run after the first takes the `pull` branch. Measured end to end at Gate C against a real published index in a real git repository: a **cold** cache puts **209 B on stderr**; a **warm** cache with the index unchanged puts `Already up to date.` — **20 B on stdout**, above Parley's own answer; a **warm** cache whose index **has moved** puts a **216 B diffstat of the index repository's own files on stdout** above an 11 B answer, plus 162 B on stderr. **So the contaminated stream is stdout, not stderr, on every run but the first** — the `publish` shape Sprint 19 existed to abolish, on the stream the original entry called clean. **The gap's own record repeated F19's mistake one level up**: it was written from the observed case rather than from the rule's units, which for this method is *both branches of `refreshCommand`*. `parley install --git <repo>` — and equally a `parley.config.st` recording `#git` — exits `0` with its correct pins, naming an internal cache path keyed by a sha256 the operator never chose and cannot act on. Sprint 19's exit criterion is *no verb prints a line Parley did not compose except fragments it deliberately keeps and documents*, and this fragment is neither removed nor documented, so **the criterion is not met as stated**. It was missed because the sprint enumerated its domain in units of *change* (five declared settled-class exceptions, three probed verbs) rather than in units of the *rule* (every class in `src/` composing a line run through the seam, which is **five**: `Publisher` ✅, the registration plan ✅, `childCommandFor:` ✅ by `-g`, `SparseIndexSource` ✅ already silent via `curl -sfS`, `GitIndexSource` ✗). **Severity: noisy, not wrong** — the results are correct and the exit codes are right; this is the Sprint 18/19 defect class (foreign text on the operator's terminal) rather than a fail-wrong. **Consumer constraint until it lands:** anyone using a git index sees two lines of git's own progress on stderr naming a Parley-internal path, and a script capturing stderr will collect them. **Estimated fix: `--quiet` on `GitIndexSource`'s two composed lines plus one law** — the decision-77 shape and the settled `curl -sfS` precedent, not a redirect. **The fix estimate survived the correction unchanged, and only the symptom was wrong.** Re-probed on **both** branches and **both** failure modes at Sprint 20's Gate C: `git clone --quiet` and `git -C <cache> pull --ff-only --quiet` each answer **0 bytes on both streams** at exit `0`; a missing repository still writes `fatal: repository '…' does not exist` at exit `128`; and a **diverged** cache still writes `fatal: Not possible to fast-forward, aborting.` at exit `128`. Silencing the chatter silences neither failure. **Scheduled as a rider on Sprint 20** (issue #22, S20–S22) — a prerequisite of nothing, so it rides and never leads (ranking rule §1.4). Note what the fix also suppresses and why that is right: `--quiet` drops the pull's diffstat, i.e. *"the index moved"* — if Parley wants to say that, it composes the line itself, and today nothing does.
...
- ~~**`check` requires `--source`**~~ — **the requirement is gone as of Sprint 21** ([decision 84](design/architecture.md#8-decision-log)), and the row is amended rather than cleared because **what shipped is not what this row deferred**. Sprint 11 refused a *weaker* source-less `check` — lock validity plus `PinVerification` only — on the ground that its retirement leg and both graphs' non-root edges need a snapshot, and declined to ship half a verb wearing a whole verb's name. Decision 84 ships the **whole** verb: the path overlay composes a real snapshot with no index present, so the leg and the edges have precisely what this row said they lacked. The objection was satisfied, not overruled. **The lock-only check this row actually described is still unbuilt and still deferred**, with its trigger unchanged and unfired: *an operator wanting a lock-only check in a context with no index reachable.* *Severity: none. No consumer constraint.*

  **The record worth keeping is how it was found.** The row was not consulted and did not fire; it was re-read at a mid-sprint halt because a ruling was being written next to it, and it read as still-correct until its *reason* was compared against the tool. A deferral row states a claim about the world, and the sprint that builds the neighbouring feature is the likeliest place for that claim to go stale — which is why decision 84's portable half is *re-read the reason, never the title*.
