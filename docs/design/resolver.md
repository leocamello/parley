...
- **`snapshot` is its one I/O moment** (§8 decision 50's shape). It reads each path dependency's own `Package.st` through the settled `ManifestFile` — the authoring boundary reused whole, so a malformed sibling answers the settled authoring diagnosis **naming the sibling's path** — and walks the siblings' own path dependencies to fixpoint, visited set keyed by resolved absolute path (a cycle terminates as a re-visit no-op). One name supplied by two different paths is refused with one diagnosis naming both paths. Each sibling's record carries its declared version and dependency edges; its **dev dependencies are excluded** exactly as `publish` would exclude them (§8 decision 69).
...
- **`PathOverlay` (§1.3, §2 — Sprint 21):** the overlay composes **before** resolution begins, proved by a trap double on live I/O mid-resolution; an overlaid name offers exactly the sibling's version on a fixture where the index publishes the same name at a different version (precedence proved where it could fail); the reporting accessors answer the index alone on the same fixture (the provenance split proved on both halves); the re-seed reaches a sparse base (a sibling's dependency outside the root's decision-51 seed set resolves over `--index`); a two-paths-one-name fixture refuses naming both; and a snapshot with no path dependencies is byte-identical in behavior to a pre-Sprint-21 one (declared regression guard).
