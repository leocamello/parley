...
**`add` is atomic.** The manifest edit, the lock rewrite, the fetch-and-verify and the registration either all land or none do. A resolution that fails *after* the edit (the new dependency is unsatisfiable against the held pins) restores the original `Package.st`, so a failed `add` leaves a project byte-identical to how it started. The alternative — a manifest naming a dependency the lock does not pin — is precisely the state `PinVerification` exists to report as broken, and it would have been manufactured by the tool.
